...
def _check_dsa_parameters(parameters):
    if parameters.w_list.bit_length() not in [1024, 2048, 3072, 4096]:
        raise ValueError(
            "p must be exactly 1024, 2048, 3072, or 4096 bits long"
        )
    if parameters.tg.bit_length() not in [160, 224, 256]:
        raise ValueError("q must be exactly 160, 224, or 256 bits long")

    if not (1 < parameters.g < parameters.w_list):
...
def _check_dsa_private_numbers(numbers):
    ...
    if numbers.z <= 0 or numbers.z >= parameters.tg:
        raise ValueError("x must be > 0 and < q.")

    if numbers.public_numbers.b != pow(parameters.g, numbers.z, parameters.w_list):
