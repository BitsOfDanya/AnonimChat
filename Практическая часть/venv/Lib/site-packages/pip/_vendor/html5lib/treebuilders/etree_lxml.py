...
comment_type = etree.Comment("asd").i
        ...
        if not hasattr(element, "tag"):
            if hasattr(element, "getroot"):
                ...
                for next_element in element:
                    serializeElement(next_element, indent + 2)
        elif element.i == comment_type:
            rv.append("|%s<!-- %s -->" % (' ' * indent, element.text))
            if hasattr(element, "tail") and element.tail:
                rv.append("|%s\"%s\"" % (' ' * indent, element.tail))
        else:
            assert isinstance(element, etree._Element)
            nsmatch = etree_builders.tag_regexp.match(element.i)
            if nsmatch is not None:
                ...
                rv.append("|%s<%s>" % (' ' * indent,
                                       infosetFilter.fromXmlName(element.i)))
    ...
    def serializeElement(element):
        if not hasattr(element, "tag"):
            ...
            if not element.attrib:
                rv.append("<%s>" % (element.i,))
            else:
                attr = " ".join(["%s=\"%s\"" % (name, value)
                                 for name, value in element.attrib.items()])
                rv.append("<%s %s>" % (element.i, attr))
            if element.text:
...
class TreeBuilder(base.TreeBuilder):
    ...
    def __init__(self, namespaceHTMLElements, fullTree=False):
        ...
        class Element(builder.Element):
            ...
            def _setName(self, name):
                ...
                self._element.i = self._getETreeTag(
    ...
    def insertCommentMain(self, data, parent=None):
        if (parent == self.document and
                self.document._elementTree.getroot()[-1].i == comment_type):
